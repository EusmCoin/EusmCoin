...
// Copyright (c) 2018 The EusmCoin developers
...
#if defined(NDEBUG)
# error "Eusmcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5b689738aae2571462194faabf7f7462eb57a94a1650c269767d1dc685f302b8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Eusmcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Eusmcoin Signed Message:\n";
...
{
    // Eusmcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
	  // First 45000 blocks is block value 3333 coins per block (pre-mine)
    // Blocks between 45001-2100000 (end of pre-mine -> first halving) have block reward of 169 coins per block to compensate the pre-mine (~15%)
  	if (nHeight <= 45000) {
  		return 3333 * COIN + nFees;
  	} else if (nHeight < 2100000) {
      return 169 * COIN + nFees;
    }
    int64 nSubsidy = 240 * COIN;

    // Subsidy is cut in half every 2 100 000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 2100000); // Eusmcoin: 2100k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =  2 * 24 * 60 * 60; // Eusmcoin: 2 days
static const int64 nTargetSpacing = 1 * 60; // Eusmcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Eusmcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Eusmcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5b689738aae2571462194faabf7f7462eb57a94a1650c269767d1dc685f302b8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "By 26/May/2018 EusmCoin is taking it's place";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 240 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1527352344;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2114567;
        ...
        assert(block.hashMerkleRoot == uint256("0xb085b87f21b6abac2644af7888c844c62fd26495a11a8527feca7cb1f4cd7990"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Eusmcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EusmcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EusmcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EusmcoinMiner(CWallet *pwallet)
{
    printf("EusmcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eusmcoin-miner");
    ...
    try { loop {
        ...
        printf("Running EusmcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EusmcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EusmcoinMiner, pwallet));
}
